...
- [ ] T031 [US3] **PARTIAL — needs Docker for the definitive number, but the finding is already clear.** Backend: distroless/static ~2 MB + an 11 MB (amd64) / 10 MB (arm64) stripped static binary, both measured — comfortably under 150 MB. Frontend: `node:22-bookworm-slim` is ~80 MB of compressed layers per the registry, plus ~68 MB of `.next/standalone` and `.next/static` measured locally. **The frontend image will not meet SC-006's 150 MB.** Reported rather than worked around, per the task
